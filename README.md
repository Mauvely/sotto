...
Windows has a real (experimental) build — see below — but no shipped
installer yet. `cmake/MauvelyPackaging.cmake` — the shared CPack/WiX module
also used by Compose and Snap — is wired into `CMakeLists.txt`, so producing
a `.msi` is a `cpack -G WIX` away once the port has had enough
real-hardware time to be worth shipping.
